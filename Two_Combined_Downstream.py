...
class DownstreamNet_Combined(nn.Module):
    def __init__(self, stager1, stager2, classes):
        super(DownstreamNet_Combined, self).__init__()
        self.stagenet1=stager1
        self.stagenet2=stager2
        self.linear = nn.Linear(200,classes) # how many labels
...
def train_end_to_end(model_path_1, model_path_2, train_set, test_set, pos_labels_per_class, max_epochs, classes, verbose=False):
    ...
    trained_model_1 = StagerNet(11)
    trained_model_1.load_state_dict(torch.load("models"+os.sep+model_path_1))
    for p in trained_model_1.parameters():
            p.requires_grad = False
            
    trained_model_2 = StagerNet(11)
    trained_model_2.load_state_dict(torch.load("models"+os.sep+model_path_2))
    for p in trained_model_2.parameters():
            p.requires_grad = False
            
    # cuda setup if allowed
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu") # PyTorch v0.4.0
    model = DownstreamNet_Combined(trained_model_1, trained_model_2, classes).to(device)
